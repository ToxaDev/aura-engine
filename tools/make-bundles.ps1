...
.DESCRIPTION
    ...
    Four tiers, chosen so the ladder is obvious:

      5k   Compact    every FS multiplier, the whole app in a few megabytes.
                      Five thousand taps with the window fitted to that length:
                      the rejection of the long filters, a gentler wall.
...
.EXAMPLE
    # Build the four bundles into .\dist
...
param(
    ...
    [ValidateSet('5k', '1M', '5M', '10M', '30M')]
    [string[]]$Tiers = @('5k', '1M', '10M', '30M'),
...
# the compact one, at 40 KB a blob, gets it for nothing.
$Fs8Rates = @(352800, 384000)
$AllRates = @(88200, 96000, 176400, 192000, 352800, 384000, 705600, 768000)

$TierSpec = [ordered]@{
    '5k'  = @{ Name = 'Compact';   Rates = $AllRates; Blurb = 'every FS multiplier (FS2-FS16)' }
...
function New-BundleReadme([string]$Tier, [string]$Label, [string]$Blurb, [string]$Tag) {
    ...
    if ($TierSpec[$Tier].Rates.Count -eq $AllRates.Count) { $rates = 'every FS multiplier - FS2, FS4, FS8 and FS16, both rate families' }
    return @(
