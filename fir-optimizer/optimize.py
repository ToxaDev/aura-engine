...
def fmt_taps(n):
    # Lowercase k, uppercase M: the tags the runtime looks for ("5k", "1M").
    if n >= 1_000_000:
        v = n / 1_000_000
...
def get_n_fft(N):
    ...
    Never below 2^23 points. A short filter needs the headroom, and it is
    cheap to give: at 2^17 (16 x 5 000) a 5k filter at 16x gets 5 Hz bins,
    too coarse to find its stopband lobes, and the cepstral minimum-phase
    conversion aliases — at 2^20 the 5k filter at 96 kHz still came out with
    1.7e-4 dB of ripple at the band edge, at 2^23 with 4e-7. From 524 288
    taps up the floor is already met, so no shipped 1M-30M filter changes.
    """
    if N < 1_000_000:
        mult = 16
    elif N < 5_000_000:
        ...
        mult = 2
    return max(2 ** int(np.ceil(np.log2(mult * N))), 1 << 23)
...
# The window every filter of a million taps and more is designed with. At that
# length β = 14 already puts the stopband near the float64 floor, and the
# shipped 1M-30M blobs were generated with it — changing it would change them.
KAISER_BETA_LONG = 14.0

# Below this length β = 14 is far too shallow (5 000 taps at 8x give -145 dB),
# so the window is fitted to the length instead: see short_filter_beta().
SHORT_FILTER_MAX_TAPS = 1_000_000

# What a short filter aims for from the stopband edge up: the rejection the
# 30M filter measures. Where the length cannot reach it, it gets the deepest
# rejection that length allows.
SHORT_FILTER_TARGET_DB = -220.0


def _worst_stopband_db(h, fs, f_stop, n_fft):
    H = np.abs(sfft.rfft(h, n=n_fft))
    freqs = np.fft.rfftfreq(n_fft, 1.0 / fs)
    return float(20 * np.log10(H[freqs >= f_stop].max() + 1e-300))


def short_filter_beta(num_taps, f_pass, f_stop, fs, target_db=SHORT_FILTER_TARGET_DB):
    """Kaiser β for a filter too short for β = 14, chosen by measurement.

    The cutoff stays where every other filter has it — the middle of the
    band — so a short filter differs from a long one in how gradual its wall
    is, not in where the wall stands. What is fitted is the window: the
    smallest β whose worst stopband, from `f_stop` up, is at or below
    `target_db`. Smallest, because a wider window than necessary only softens
    the wall further.

    Measured rather than taken from the Kaiser design formula, which at
    220 dB overestimates what a given β achieves by about 13 dB.

    The worst stopband is unimodal in β: it falls while the sidelobes set it,
    then rises once the widening main lobe reaches `f_stop`. When even its
    minimum misses the target, that minimum is used and the shortfall shows
    up in the exported stats.

    Returns (β, worst stopband in dB) with β rounded to 0.01.
    """
    # The search runs on a coarser grid than the final analysis; the answer is
    # then re-checked at full resolution, where a lobe peak can sit a fraction
    # of a dB higher.
    def worst_at(beta, n_fft):
        h = _generate_linear_phase_kaiser_scipy_fast(num_taps, f_pass, f_stop, fs, beta=beta, quiet=True)
        return _worst_stopband_db(h, fs, f_stop, n_fft)

    def worst(beta):
        return worst_at(beta, 1 << 20)

    # Coarse scan for the deepest point, then a golden-section refinement.
    grid = np.arange(8.0, 45.0 + 1e-9, 0.5)
    depths = [worst(b) for b in grid]
    ...
    for _ in range(25):
        ...
        if worst(a) < worst(b):
            ...
            lo = a
    beta_deepest = round((lo + hi) / 2, 2)
    deepest = worst(beta_deepest)
    if deepest > target_db:
        return beta_deepest, worst_at(beta_deepest, get_n_fft(num_taps))

    # The target is reachable: bisect down the falling side for the smallest β.
    lo, hi = 8.0, beta_deepest
    for _ in range(40):
        mid = (lo + hi) / 2
        if worst(mid) <= target_db:
            hi = mid
        else:
            lo = mid
    beta = float(np.ceil(hi * 100) / 100)
    full = get_n_fft(num_taps)
    while beta < beta_deepest and worst_at(beta, full) > target_db:
        beta = round(beta + 0.01, 2)
    beta = min(beta, beta_deepest)
    return beta, worst_at(beta, full)
...
def _generate_linear_phase_kaiser_mpmath_legacy(num_taps, f_pass, f_stop, fs, beta=KAISER_BETA_LONG):
...
def _generate_linear_phase_kaiser_scipy_fast(num_taps, f_pass, f_stop, fs, beta=KAISER_BETA_LONG, quiet=False):
...
def generate_linear_phase_kaiser(num_taps, f_pass, f_stop, fs, beta=KAISER_BETA_LONG):
    ...
    if _legacy_mpmath_enabled():
        return _generate_linear_phase_kaiser_mpmath_legacy(num_taps, f_pass, f_stop, fs, beta=beta)
    return _generate_linear_phase_kaiser_scipy_fast(num_taps, f_pass, f_stop, fs, beta=beta)
...
def analyze_filter(h_np, fs, f_pass, f_stop):
    ...
    # Where the response first leaves ±0.01 dB — how far up the band is flat.
    # For a million taps and more this sits within a few Hz of the cutoff;
    # for a short filter it is the number that says how gradual the wall is.
    off = np.abs(mag_db) > 0.01
    flat_to = float(freqs[np.argmax(off)]) if off.any() else float(fs / 2)
...
def generate(config, adaptive_analysis=None):
    ...
    beta = KAISER_BETA_LONG
    if num_taps < SHORT_FILTER_MAX_TAPS:
        print("[0/3] Fitting the Kaiser window to a short filter...")
        t0 = time.time()
        beta, reached = short_filter_beta(num_taps, f_pass, f_stop, fs)
        verdict = "target met" if reached <= SHORT_FILTER_TARGET_DB else "deepest this length allows"
        print(f"      beta={beta:.2f} -> worst stopband {reached:.1f} dB from {f_stop:.0f} Hz "
              f"({verdict}) in {time.time()-t0:.1f}s")
    config['design'] = {
        'window': 'kaiser',
        'beta': float(beta),
        'cutoff_hz': float((f_pass + f_stop) / 2.0),
    }

    print("[1/3] Generating linear-phase Kaiser...")
    t0 = time.time()
    h_linear = generate_linear_phase_kaiser(num_taps, f_pass, f_stop, fs, beta=beta)
...
def interactive_menu(config):
    while True:
        ...
        print(f"      (5 sizes × 2 sources × 4 multipliers × 2 phases = 80 .npy files,")
...
def main():
    ...
    parser.add_argument('--all-ratios', action='store_true',
                        help='Batch-generate FIR pairs for every (size × source × multiplier) '
                             'combination the desktop converter can request: 5 sizes × 2 source '
                             'rates × 4 multipliers × 2 phases = 80 .npy files. Skips files that '
...
#
#   sizes (taps):   5k, 1M, 5M, 10M, 30M (5 entries — match Rust TAP_LADDER)
...
#      fir_5k_352800_linear_phase.npy    (44.1 × 8)
#
...
#
# Every size shares that band and its midpoint cutoff. From 1M up the window
# is Kaiser β = 14; the 5k filter has its β fitted per rate by
# short_filter_beta() — it keeps the cutoff and gives up only how steep the
# wall is (see docs/12).

ALL_RATIOS_SIZES = [5_000, 1_000_000, 5_000_000, 10_000_000, 30_000_000]
...
def _band_for_source(source_rate: int):
    ...
    Yields ~2 kHz transition band. The measured stopband is in each blob's
    meta.json: about -185 to -203 dB at 1M, -204 to -221 dB at 30M, and
    -220 dB at 5k wherever that length can reach it (FS2-FS8).
    """
    nyq = source_rate / 2.0
    f_pass = nyq - 2000.0
    f_stop = nyq
