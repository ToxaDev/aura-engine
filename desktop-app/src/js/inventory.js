...
//
// Settings store the tap count itself, not the slider position: positions
// shift whenever a rung is added, and 1.2.7 added one below the old bottom.
export const TAP_PRESETS = [5000, 1000000, 5000000, 10000000, 30000000];
export const FS_PRESETS = [2, 4, 8, 16];

// What a saved tap-slider position meant before 1.2.7, when settings stored
// the position rather than the tap count. Position 0 was 1M then and is 5k now,
// so an old position read against the new ladder would quietly move every
// returning user one rung down.
const TAP_PRESETS_BEFORE_1_2_7 = [1000000, 5000000, 10000000, 30000000];

/// Slider position for a saved setting, or null when there is nothing usable.
export function tapIndexFromSaved(s) {
    const count = parseInt(s.convTapCount);
    if (TAP_PRESETS.includes(count)) return TAP_PRESETS.indexOf(count);
    if (s.convTaps !== undefined && s.convTaps !== null) {
        const old = parseInt(s.convTaps);
        if (Number.isInteger(old)) {
            const taps = TAP_PRESETS_BEFORE_1_2_7[Math.min(Math.max(old, 0), 3)];
            return TAP_PRESETS.indexOf(taps);
        }
    }
    return null;
}
...
/// both branches in full, and TFS derives its filter from the pair, so for
/// either of them half a pair is not enough.
