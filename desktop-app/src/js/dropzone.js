...
export function renderFileQueue(changedIdxs = null) {
    ...
    const getProps = (f) => {
        ...
        // SUB runs on every file of the batch it was set for, so its badge is
        // the setting, like HP's — first, because the filter runs first.
        if (f.sub && f.batch !== 'next' && f.badge !== 1 && f.badge !== 2) badges.push(`<span class="file-item-badge badge-sub" title="Subsonic filter: linear-phase high-pass, flat from ${f.sub} Hz up">SUB${f.sub}</span>`);
        // AA badge is per-file: lit green when the Adaptive Apodizer treated
        // the track, dimmed grey ("lamp off") when it analyzed the track and
        // deliberately left it untouched — the tooltip carries the reason.
        if (f.aa && f.batch !== 'next' && f.badge !== 1 && f.badge !== 2) {
            const aaTip = (f.aaNote || '').replace(/"/g, '&quot;');
            if (f.aaState === 2) {
                badges.push(`<span class="file-item-badge badge-aa badge-aa-off" title="${aaTip || 'Adaptive Apodizer: this track does not need treatment.'}">AA</span>`);
            } else if (f.aaState === 1) {
                badges.push(`<span class="file-item-badge badge-aa" title="${aaTip || 'Adaptive Apodizer: treatment applied.'}">AA</span>`);
            } else {
                badges.push('<span class="file-item-badge badge-aa" title="Adaptive Apodizer: analyzing source...">AA</span>');
            }
        }
