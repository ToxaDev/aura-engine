...
import {
    ...
    tapIndexFromSaved, TAP_PRESETS, FS_PRESETS
...
test('a fresh install opens on the filter its bundle shipped', (t) => {
    ...
    // The 5k bundle carries every multiplier and still opens at FS8.
    install([[K5, ALL_RATES]]);
    assert.deepEqual(bestCombo(false), { taps: K5, fs: 8 });
});

test('the 5k bundle unzipped beside a larger one does not take over', () => {
    install([[K5, ALL_RATES], [M10, FS8_RATES]]);
    assert.deepEqual(bestCombo(false), { taps: M10, fs: 8 });
    // ...but it keeps every multiplier the larger bundle lacks within reach.
    assert.deepEqual(nearestAvailable(M10, 16, false), { taps: M10, fs: 8 });
    assert.deepEqual(nearestAvailable(K5, 16, false), { taps: K5, fs: 16 });
});
...
test('a full matrix still opens on 30M at FS8, as it always did', () => {
    install([[K5, ALL_RATES], [M1, ALL_RATES], [M5, ALL_RATES], [M10, ALL_RATES], [M30, ALL_RATES]]);
    assert.deepEqual(bestCombo(false), { taps: M30, fs: 8 });
});
...
test('the ladder starts at 5k and keeps the long rungs in order', () => {
    assert.deepEqual(TAP_PRESETS, [K5, M1, M5, M10, M30]);
});

test('a tap setting saved as a slider position before 1.2.7 keeps its filter', () => {
    // Position 0..3 meant 1M/5M/10M/30M. Read against the new ladder
    // unchanged, every returning user would drop one rung — 30M to 10M.
    const cases = [[0, M1], [1, M5], [2, M10], [3, M30], ['3', M30], [9, M30]];
    for (const [pos, taps] of cases) {
        assert.equal(TAP_PRESETS[tapIndexFromSaved({ convTaps: pos })], taps, `old position ${pos}`);
    }
});

test('a tap setting saved as a count is taken as it is', () => {
    for (const taps of TAP_PRESETS) {
        assert.equal(TAP_PRESETS[tapIndexFromSaved({ convTapCount: taps })], taps);
    }
    // The count wins over a leftover position from an older save.
    assert.equal(TAP_PRESETS[tapIndexFromSaved({ convTapCount: K5, convTaps: 3 })], K5);
    // A count off the ladder falls back to the position, then to nothing.
    assert.equal(TAP_PRESETS[tapIndexFromSaved({ convTapCount: 65536, convTaps: 1 })], M5);
    assert.equal(tapIndexFromSaved({ convTapCount: 65536 }), null);
    assert.equal(tapIndexFromSaved({}), null);
});
