...
/// instead of materializing the whole file. Same f64 extraction as the batch
/// decoder in decode.rs (exact for 16/24-bit integer PCM).
fn decode_via_symphonia_streaming(path: &Path, cmp: &mut StreamCompare) -> Result<(), String> {
    ...
    loop {
        ...
        for frame in interleaved.chunks_exact(ch) {
            let l = frame[0];
            chunk_l.push(l);
            chunk_r.push(if ch >= 2 { frame[1] } else { l });
        }
