...
pub struct FileConvState {
    ...
    /// Lab chain for this file as the queue row draws it: a JSON array of
    /// `["TOK", state, "why"]`, state 1 = ran, 2 = reached and declined,
    /// 3 = failed. Written twice — once the source passes are done, and
    /// again at the end — so the row fills in while the file converts
    /// instead of appearing all at once when it finishes.
    pub lab_chain: Mutex<String>,
...
impl FileConvState {
    pub fn new() -> Self {
        Self {
            ...
            lab_chain: Mutex::new(String::new()),
    ...
    /// Publish the chain and mark the file changed, so the next poll ships it.
    pub fn set_lab_chain(&self, json: String) {
        *self.lab_chain.lock().unwrap() = json;
        self.touch();
    }
    pub fn touch(&self) {
