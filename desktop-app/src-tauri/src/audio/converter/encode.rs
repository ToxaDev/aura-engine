...
pub fn build_output_name(
    ...
    let taps_str = if settings.taps >= 1_000_000 {
        ...
        // Lowercase, like the rates beside it and the blob tag ("5k").
    ...
    // Format: [AE · 44.1k→384k · Kaiser 10M · f64 · DC·SUB20·AA·PFR·HP · 6ch→2.0]
    ...
    // A static apodizing preset is not a stage of the chain and has no
    // badge; it keeps its own segment. The adaptive one does have a badge and
    // arrives as the "AA" token below, in the place where it ran.
    // The subsonic corner is not a segment of its own here, unlike
    // upstream: it is a stage of the chain and arrives as SUB20/SUB15/SUB10
    // below, pushed by prepare.rs in the place where it ran.
    if let Some(tag) = apod_tag {
        tags.push(tag.to_string());
    }

    // The chain: every stage that ACTUALLY fired, in the order the pipeline
    // ran them, joined by middle dots. This replaced a `LAB[...]` bracket
    // holding only the lab passes while AA and HP sat outside it in settings
    // order — which said what had been switched on, not what had happened.
    if !lab_chain.is_empty() {
        let ordered = crate::audio::converter::dsp::lab::in_chain_order(lab_chain);
        tags.push(ordered.join("\u{00B7}"));
    }
...
/// Serialize a VORBIS_COMMENT metadata block payload (vendor string +
/// comment entries). Returns the raw payload bytes (not including the
/// 4-byte block header).
fn serialize_vorbis_comment(tags: &[(String, String)]) -> Vec<u8> {
    let vendor = b"AuraEngine";
    let mut out = Vec::new();
    // 4-byte LE vendor string length
    let vlen = vendor.len() as u32;
...
impl StreamingFlacEncoder {
    /// Tagless constructor — kept as the stable legacy entry point (used by
    /// the encoder round-trip tests); production paths go through new_tagged.
    ...
    /// Construct encoder with VORBIS_COMMENT tags. Tags are written as a
    /// metadata block immediately after STREAMINFO. An empty slice produces
    /// the old single-block layout byte-for-byte.
    pub fn new_tagged(
        sample_rate: u32,
        output_path: &Path,
        tags: &[(String, String)],
        ...
        Self::with_candidates(sample_rate, output_path, block_size, FLAC_CANDIDATES, tags)
    }
    ...
    fn with_candidates(
        ...
        // fLaC magic. When tags are present: STREAMINFO is NOT the last
        // metadata block (0x00 instead of 0x80); VORBIS_COMMENT follows as
        // the last block (0x84). When tags are absent the old layout is
        // preserved byte-for-byte (0x80 = last).
        let streaminfo_hdr_byte: u8 = if tags.is_empty() { 0x80 } else { 0x00 };
        file.write_all(&[0x66, 0x4C, 0x61, 0x43, streaminfo_hdr_byte, 0x00, 0x00, STREAMINFO_LEN])
        ...
        // count and the frame-size extremes are known. STREAMINFO_OFFSET=8
        // stays correct because the STREAMINFO payload still starts at byte 8.
        let info = enc.serialize_stream_info()?;
        enc.write_all(&info)?;

        // VORBIS_COMMENT block (type 4, last-metadata = 1 → 0x84).
        // Written only when tags are supplied; otherwise the file is
        // byte-identical to the old tagless layout.
        if !tags.is_empty() {
            let payload = serialize_vorbis_comment(tags);
            let plen = payload.len();
            if plen > 0x00_FF_FF_FF {
                return Err("VORBIS_COMMENT payload exceeds FLAC metadata block limit".to_string());
            }
            let b0 = ((plen >> 16) & 0xFF) as u8;
            let b1 = ((plen >> 8) & 0xFF) as u8;
            let b2 = (plen & 0xFF) as u8;
            // 0x84 = last-metadata-block (bit 7) | type 4 (VORBIS_COMMENT)
            enc.write_all(&[0x84, b0, b1, b2])
                .map_err(|e| format!("FLAC VORBIS_COMMENT header write error: {}", e))?;
            enc.write_all(&payload)
                .map_err(|e| format!("FLAC VORBIS_COMMENT payload write error: {}", e))?;
        }
...
/// Encode f64 PCM to FLAC, optionally writing VORBIS_COMMENT metadata.
/// An empty `tags` slice produces a file byte-identical to the old tagless path.
pub fn encode_flac(
    samples_l: &[f64],
    samples_r: &[f64],
    sample_rate: u32,
    ...
    let mut enc = StreamingFlacEncoder::new_tagged(sample_rate, output_path, tags)?;
...
mod tests {
    ...
    /// VORBIS_COMMENT block: verify fLaC magic, STREAMINFO is non-last when
    /// tags present, VORBIS_COMMENT block header type is 4 and is last, and
    /// that the encoded file is still decoded correctly by Symphonia.
    #[test]
    fn vorbis_comment_block_layout() {
        let n = 8192;
        let (sig_l, sig_r) = (grid_signal(n, 0.0), grid_signal(n, 0.5));
        let path = std::env::temp_dir().join("ae_encode_vc_layout.flac");

        let tags = vec![
            ("AURAENGINE_LAB".to_string(), "1".to_string()),
            ("AURA_CHAIN".to_string(), "DC\u{00B7}ISP".to_string()),
        ];
        let mut enc = StreamingFlacEncoder::new_tagged(352_800, &path, &tags).expect("encoder");
        enc.feed(&sig_l, &sig_r).expect("feed");
        enc.finish().expect("finish");

        let raw = std::fs::read(&path).expect("read encoded file");
        std::fs::remove_file(&path).ok();

        // fLaC magic
        assert_eq!(&raw[0..4], b"fLaC", "fLaC magic missing");

        // Byte 4: STREAMINFO block header — must NOT be last (bit 7 = 0)
        let streaminfo_hdr = raw[4];
        let block_type = streaminfo_hdr & 0x7F;
        let is_last = (streaminfo_hdr & 0x80) != 0;
        assert_eq!(block_type, 0, "block after fLaC must be STREAMINFO (type 0)");
        assert!(!is_last, "STREAMINFO must not be last when tags present");

        // Skip to the block after STREAMINFO: byte 4 (hdr) + 3-byte length
        let si_len = (((raw[5] as usize) << 16) | ((raw[6] as usize) << 8) | raw[7] as usize);
        assert_eq!(si_len, STREAMINFO_LEN as usize, "STREAMINFO length field");
        let vc_hdr_offset = 4 + 4 + si_len;

        // VORBIS_COMMENT block header: 0x84 = last + type 4
        let vc_hdr = raw[vc_hdr_offset];
        assert_eq!(vc_hdr & 0x7F, 4, "second block must be VORBIS_COMMENT (type 4)");
        assert_eq!(vc_hdr & 0x80, 0x80, "VORBIS_COMMENT must be the last metadata block");

    }

    /// Empty tags → file is byte-identical to the old tagless path (no extra block).
    #[test]
    fn empty_tags_produce_single_streaminfo_block() {
        let n = 4096;
        let (sig_l, sig_r) = (grid_signal(n, 0.0), grid_signal(n, 0.5));
        let path = std::env::temp_dir().join("ae_encode_no_tags.flac");
        ...
        // fLaC magic
        assert_eq!(&raw[0..4], b"fLaC");
        // STREAMINFO is the last metadata block (0x80)
        assert_eq!(raw[4] & 0x80, 0x80, "STREAMINFO must be last when no tags");
        assert_eq!(raw[4] & 0x7F, 0, "block type must be STREAMINFO");
    }

    /// The subsonic corner rides in the chain like every other stage, in
    /// the place the pipeline runs it — after the repairs, before AA — and a
    /// file converted without it is named as before.
    #[test]
    fn output_name_carries_subsonic_corner_in_chain_order() {
        let audio = AudioFile {
            samples_l: vec![],
            samples_r: vec![],
            sample_rate: 44_100,
        ...
        let src = Path::new("03 Mal Bicho.flac");
        assert_eq!(
            build_output_name(&audio, &s, src, 352_800, None, &["HP", "AA", "SUB20", "DC"]),
            "03 Mal Bicho [AE · 44.1k→352.8k · Kaiser 10M · f64 · DC\u{00B7}SUB20\u{00B7}AA\u{00B7}HP].flac"
        );
        assert_eq!(
            build_output_name(&audio, &s, src, 352_800, None, &["HP", "AA"]),
            "03 Mal Bicho [AE · 44.1k→352.8k · Kaiser 10M · f64 · AA\u{00B7}HP].flac"
        );
    }
}
