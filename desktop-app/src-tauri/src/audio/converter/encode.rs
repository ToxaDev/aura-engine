...
pub fn build_output_name(
    ...
    // Format: [AE · 44.1k→384k · Kaiser 10M · f64 · SUB20 · AA · HP · 6ch→2.0]
    ...
    // Subsonic filter — "SUB20": high-passed below 20 Hz, flat from 20 Hz up.
    // It runs on every file whenever a corner is set, so the setting is the fact.
    if settings.subsonic_hz != 0 {
        tags.push(format!("SUB{}", settings.subsonic_hz));
    }
...
mod tests {
    ...
    /// The subsonic corner is in the name, before AA and HP, because the
    /// filter runs before both; a file converted without it is named exactly
    /// as it was before the option existed.
    #[test]
    fn output_name_carries_subsonic_corner_in_pipeline_order() {
        let audio = AudioFile {
            samples_l: vec![],
            samples_r: vec![],
            sample_rate: 44_100,
        ...
        assert_eq!(
            build_output_name(&audio, &s, src, 352_800, Some("AA")),
            "03 Mal Bicho [AE · 44.1k→352.8k · Kaiser 10M · f64 · AA · HP].flac"
        );
        s.subsonic_hz = 20;
        assert_eq!(
            build_output_name(&audio, &s, src, 352_800, Some("AA")),
            "03 Mal Bicho [AE · 44.1k→352.8k · Kaiser 10M · f64 · SUB20 · AA · HP].flac"
        );
        s.subsonic_hz = 10;
        s.hybrid_phase = false;
        assert_eq!(
            build_output_name(&audio, &s, src, 352_800, None),
            "03 Mal Bicho [AE · 44.1k→352.8k · Kaiser 10M · f64 · SUB10].flac"
        );
    }
}
