...
/// user who is missing one needs the pack name, not just the filename. The 5k
/// pack is the same scheme at a size that barely registers: sixteen blobs of
/// 40 KB each.
...
pub fn missing_filter_error(taps: usize, target_rate_hz: u32, phase_type: &str) -> String {
    ...
    let (wanted, wanted_for) = match taps_label(taps) {
        ...
        None => (
            format!(
                "no filter is designed for {} taps — the smallest is {}",
                taps,
                taps_label(TAP_LADDER[0]).unwrap_or("?")
            ),
            format!(
                "{} taps at {} Hz output ({}).",
...
///
/// The jump from 5k to 1M is deliberate. Five thousand taps, with the window
/// fitted to that length, already meet the rejection the long filters have
/// (docs/12); the long rungs differ from it in how steep the wall at the
/// cutoff is, and nothing between the two would add a meaningful step.
pub const TAP_LADDER: [usize; 5] = [5_000, 1_000_000, 5_000_000, 10_000_000, 30_000_000];
...
mod tests {
    ...
    fn download_pack_matches_the_missing_blob() {
        ...
        assert_eq!(
            filter_pack_for(5_000, 705_600).unwrap().0,
    ...
    /// The bottom rung is five thousand taps, not a million. Its blobs carry a
    /// lowercase `k` — the tag the generator writes — and a request below it
    /// must still be refused by name rather than rounded up to it.
    #[test]
    fn five_thousand_taps_is_its_own_rung() {
        assert_eq!(TAP_LADDER[0], 5_000);
        assert_eq!(
            blob_name(5_000, 352_800, "linear_phase").as_deref(),
            Some("fir_5k_352800_linear_phase.npy")
        );
        assert_eq!(taps_label(1_000), None);
        let msg = missing_filter_error(1_000, 352_800, "linear_phase");
        assert!(msg.contains("the smallest is 5k"), "{msg}");
    }
    ...
    fn tap_count_packs_cover_both_rate_families() {
        ...
        for taps in [5_000, 1_000_000, 5_000_000, 10_000_000] {
