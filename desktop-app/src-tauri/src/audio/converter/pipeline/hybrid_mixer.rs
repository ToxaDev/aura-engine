...
pub fn apply_hybrid_phase(
    ...
    hpss_src_l: &[f64], hpss_src_r: &[f64], hpss_src_sr: u32
        ...
        let id = crate::audio::gpu::filter_cache::FilterId::whole(&min_path_str);
        let mut dsp_min: Box<dyn DspProcessor + Send> = crate::audio::gpu::build_convolver(
            &min_coeffs,
            settings.precision,
            Some(&id),
            settings.use_gpu,
        ...
        // In-place blend: the result replaces final_l/final_r without
        // allocating two more full-length output buffers.
        ...
        final_l.truncate(len);
        final_r.truncate(len);
        if settings.lab.continuous_alpha {
            crate::aelog!("[ALPHA] Continuous hybrid blend enabled (standard path)");
            crate::audio::hybrid_phase::blend_outputs_stereo_continuous(
                &mut final_l, &min_l, &mut final_r, &min_r, &envelope, 0, out_sr_f64,
            );
        } else {
