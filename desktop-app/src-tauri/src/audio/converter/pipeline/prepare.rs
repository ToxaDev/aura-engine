...
pub fn prepare_audio_phase(
    ...
    //
    // Mean subtraction above removes a constant offset and nothing else; a
    // slow wobble — 1 Hz at 1 % of full scale on one disc in the ASR thread —
    // goes through it untouched. This is the linear-phase high-pass for
    // that, at the source rate and before anything that analyses or filters
    // the signal, so the apodizer's forensics and the upsampling filter see
    // the source without it. Flat from the corner up, so nothing above it
    // moves in level or in time. Spec and tests: `apply_subsonic_filter`.
