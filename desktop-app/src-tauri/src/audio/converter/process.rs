...
pub fn process_one_prepared(
    ...
    if use_integrated {
        ...
        if crate::audio::converter::pipeline::segmented::is_giant_plan(
            ...
            let hybrid_data: Option<(Vec<Vec<f64>>, f64, usize, usize, String)> = if settings.hybrid_phase
            {
                let min_path_str =
            ...
            let output_name =
                build_output_name(&audio, settings, src_path, out_rate, apod_tag.as_deref());
            let output_dir = src_path.parent().unwrap_or(Path::new("."));
            let hybrid_params = hybrid_data.as_ref().map(|(p, s, f, t, path)| {
                crate::audio::converter::pipeline::segmented::GiantHybridParams {
                    min_phases: p,
                    min_scale: *s,
                    min_flush_input: *f,
                    min_trim: *t,
            ...
            return crate::audio::converter::pipeline::segmented::convert_giant_integrated(
                src_path,
                &audio_l,
                &audio_r,
                total_input_samples,
