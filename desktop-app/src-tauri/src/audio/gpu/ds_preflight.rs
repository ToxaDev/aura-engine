...
/// Build a dedicated wgpu device that *requires* the SPIRV passthrough
/// feature. Deliberately not the app's shared context: that one treats
/// passthrough as optional and reports its absence, while this test exists to
/// prove what the driver does with the feature present, and would be
/// meaningless on a device that quietly went without it.
