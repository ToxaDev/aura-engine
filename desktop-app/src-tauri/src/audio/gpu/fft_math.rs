...
impl GpuDspProcessor {
    ...
    pub(crate) fn compute_h_blocks_cpu_ds_f64(
        ...
        use rayon::prelude::*;
        // One plan, shared by every partition. Planning an N = 4M transform
        // materialises a ~32 MB twiddle table; doing it inside the rayon
        // closure built one per partition (K = 15 at 30M taps → ~480 MB of
        // duplicate tables, plus the planning time itself). rustfft's Fft
        // trait is Send + Sync and `process` takes &self, so a single Arc is
        // safe to share across the workers.
