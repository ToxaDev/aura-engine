...
///   0.083 ms micro-fade, 20 ms retriggerable hold.
...
pub(crate) fn switch_params(output_sr: f64) -> (usize, usize, usize) {
    ...
    (search_window, micro_fade, min_cooldown)
}
    ...
    F: Fn(usize) -> f64 + Sync,
{
    ...
    let (search_window, micro_fade, min_cooldown) = switch_params(output_sr);
...
fn apply_switch_plan_inplace(plan: &SwitchPlan, y_linear: &mut [f64], y_minimum: &[f64]) {
    ...
    // Raised-cosine crossfade — reads both signals at idx before writing idx.
