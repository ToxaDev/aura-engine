...
```
Input File → Decode → DC Block → [Subsonic Filter] → Adaptive Apodizer
           → [Resample + FIR] → Hybrid-Phase → True Peak → Dither → FLAC → Verify
```

`[Subsonic Filter]` is optional and off by default (1.2.8, §5a).
...
| where | `pipeline/prepare.rs`, right after DC blocking, at the **source** rate, before the apodizer's forensics and everything else |
| code | `apodize.rs`: `design_subsonic_highpass`, `apply_subsonic_filter`, `fft_convolve_linear_inplace` |
...
### Off
With the option off nothing in the chain changes: `subsonic_hz == 0` skips the
block, and the output is byte-identical to 1.2.7.
...
### ⚠️ CRITICAL: Polyphase FIR Path — Spectral Imaging + Wrong Gain
