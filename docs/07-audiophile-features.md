...
- **Subsonic filter (optional, off by default, since 1.2.8).** A linear-phase FIR high-pass at the source rate, straight after DC blocking. Flat within 0.01 dB from the chosen corner up — 20, 15 or 10 Hz — and at least 100 dB down from half the corner to DC. Above the corner nothing moves in level or in time, which is the reason to do it offline: a high-pass in the playback chain cannot promise that. It is there to protect speakers from infrasound and drift a disc can carry (one CD in the Audio Science Review thread had a component near 1 Hz at about 1 % of full scale); it is not a sound improvement, and no audibility is claimed. The output filename carries `SUB20` / `SUB15` / `SUB10`. Details: [docs/05 §5a](05-converter-pipeline.md#subsonic-filter).
