...
Above the corrective cutoff the pre-ring the detector measured is gone; below the apodizer's band the tonal balance is identical to the source (null test: ±0.002 dB) — unless the subsonic filter is on, which by design removes everything below its corner. Whether removing it is audible depends on the recording and on who is listening — the project makes no claim about it. An apodizer that ran is named in the output filename (`AA`/`Apod`).
...
**The problem:** A DAC chip interpolates in real time, so its reconstruction filter is whatever fits its silicon and latency budget — typically a few hundred to a few thousand taps, with the stopband depth and transition width that budget allows.
...
**What the tap count changes:**

How steep the wall at the cutoff is, and how long a conversion takes — little else. On the magnitude response there is nothing in thirty million taps that five thousand with a wider window cannot do: the shipped 5k filter keeps the long filters' image rejection, and on music a 5k and a 30M render agree to the dither below 20.7 kHz, differing only in the last kilohertz below the source's Nyquist frequency ([docs/12 §9](12-precomputed-fir-matrix.md#short)). Nothing downstream depends on the length either. The tap count is a setting with a ceiling, not a quality claim, and nothing in this project rests on it being audible.

What the offline design buys is exactness: coefficients designed once, checked against their measured response, and applied in f64. The DAC receives a waveform that is already oversampled, so its own filter works far from the audio band.
...
- *Linear phase* has a symmetric impulse response, so energy lands both before and after every transient. What lands before it is pre-ringing: the source had nothing there. Whether it is audible is contested, and this project does not claim to have settled it — see [06-hybrid-phase-proof.md §1](06-hybrid-phase-proof.md).
- *Minimum phase* has the same magnitude response with all the energy moved past the peak, so there is no pre-ringing — at the cost of a non-linear phase response that shifts low frequencies against high ones for the whole track.
...
The onset envelope from the HPSS analysis is upsampled to the output rate using **Catmull-Rom** interpolation (C¹-continuous), eliminating the derivative kinks that caused switch-threshold jitter at the ~0.3 onset level. The `.onset_envelope.json` cache is version-tagged (`hpss_native_rust_v4`) **and** tied to a fingerprint of the source audio, so an envelope is never reused for a file that merely shares a name with the one it was computed from.

**What triggers the switch — and what does not:**

This is a detector of the **start of a sound**, not of pre-ringing. It knows nothing about the filter's transition band or how much signal energy lands in it. On music the two criteria almost always coincide, because a drum hit is both a beginning and a broadband event. On synthetic material they come apart: a steady square wave with arbitrarily steep edges produces no trigger at all, because nothing *begins*. The accurate name for the stage is "minimum phase on attacks", not "minimum phase wherever the filter would ring".

**What changes measurably:**

Inside the switching regions the output matches the minimum-phase render; outside them it is bit-identical to the linear-phase one. So ahead of the attacks that trigger the stage there is no impulse-response precursor in the output, while phase stays linear across the remaining ~85–95 % of the material. The perceptual rationale is in [DSP_MANIFESTO.md](../DSP_MANIFESTO.md) §3.1; the project makes no claim about audibility.
...
- **Lossless 24-bit decoding.** Packets are copied straight into `f64`. Integer PCM divides by 2¹⁵ / 2²³ / 2³¹ exactly, with no mantissa rounding; samples above full scale — which MP3, AAC and float WAV legitimately carry — arrive as they are rather than clipped at the door (they were clipped before 1.2.9).
...
- **Cubic Sinc interpolation and DC blocking.** The upsampling stage uses cubic sinc interpolation for sub-sample precision; before any processing, each channel's mean is subtracted — a source's constant DC offset. Only the constant: slow drift and infrasound pass through it untouched.
- **Subsonic filter (since 1.2.8; on by default at 15 Hz since 1.3.0).** A linear-phase FIR high-pass at the source rate, after the source repairs — and, since 1.3.3, a second time at the output rate after the ISP output limiter, whose gain dips would otherwise put infrasonic products back under the corner. Flat within 0.01 dB from the chosen corner up — 20, 15 or 10 Hz — and at least 100 dB down from half the corner to DC. Above the corner nothing moves in level or in time, which is the reason to do it offline: a high-pass in the playback chain cannot promise that. It is there to protect speakers from infrasound and drift a disc can carry (one CD in the Audio Science Review thread had a component near 1 Hz at about 1 % of full scale); it is not a sound improvement, and no audibility is claimed. The output filename carries `SUB20` / `SUB15` / `SUB10`. Details: [docs/05 §5a](05-converter-pipeline.md#subsonic-filter).
