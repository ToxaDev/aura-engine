...
3. The engine switches linear → minimum phase at a **zero crossing of the
   ...
   image cannot skew), with a 1 ms fade and a 20 ms anti-chatter hold.
...
The 5k rung added in 1.2.7 is a different design and does not belong on this
ladder: its window is fitted to its length instead of held at β = 14, which
buys the stopband back (−193 to −220 dB) and gives up the steepness of the
wall. It is measured in
[docs/12 §9](12-precomputed-fir-matrix.md#short).
