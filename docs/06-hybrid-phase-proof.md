...
> - **The switch fade is 1 ms** (`sr × 0.001`, on the running integral of a Hann
>   window — `switch_fade`). Until 1.3.2 it was ~0.083 ms, and that was too short:
>   every switch left a broadband tick. On a clipped CD at 176.4 k / 30M, content
>   above 24 kHz peaked at −42.9 dBFS, and all 895 ticks above −80 dBFS sat within
>   0.1 ms of a switch. With the 1 ms fade it peaks at −103.0 dBFS. The price is
>   one millisecond with both branches sounding: against a click-free splice,
>   14–21 kHz in the 3 ms around a switch is 0.32 dB lower (median), 8–14 kHz
>   0.05 dB, nothing below 8 kHz. There is no "50 ms crossfade" in the code —
>   20 ms is the `min_cooldown` (hold timer), not the fade duration.
...
>   versioned **and** tied to a fingerprint of the source audio — see §2.3.
...
>   with a 1 ms fade (a proportional blend would cause comb filtering) — see
>   `blend_outputs`.
...
**Pre-ringing** is energy placed before the transient that produced it. That is
a property of the filter, not of the recording: the source had nothing there.

Whether it is *audible* is a separate question, and an actively contested one.
The strongest argument against its relevance is that a properly bandlimited
recording contains no energy near Nyquist to excite the filter in the first
place, so a steep linear-phase filter does not measurably ring on good
material — see, for example, [Archimago's analysis of filter ringing with real
music](http://archimago.blogspot.com/2018/01/audiophile-myth-260-detestable-digital.html),
which concludes that the concern is largely misplaced and that minimum-phase
filters are frequently marketed as a cure for a non-problem.

This project does not claim to have settled that. What it claims is narrower
and testable: the Hybrid-Phase stage moves impulse-response energy from before
the attack to after it, the switch is sample-accurate and stereo-linked, and
the result nulls against the linear-phase render everywhere outside the
switching regions. Everything below is a proof of *that*, not of audibility.
...
Pre-ringing disappears. Post-ringing remains — the argument in its favour rests
on forward masking by the signal itself, and that is a rationale, not a
measurement.

### What Is Actually Detected — and What Is Not

The stage triggers on the **start of a sound**, not on pre-ringing.

The detector takes the percussive component from HPSS and measures the rise of
its energy between analysis frames. It knows nothing about the filter: not
where the transition band sits, not how much signal energy lands in it, not
whether the filter would ring on this material at all.

On music the two criteria almost always coincide — a drum hit is both a
beginning and a broadband event capable of exciting the filter. They can only
be separated deliberately, and then the gap is immediate:

| material | linear-phase pre-ringing | detector fires |
|---|---|---|
| snare hit out of silence | yes | yes |
| steady square wave, arbitrarily steep edges | yes, on every edge | **no** — nothing begins |
| steady cymbal wash | yes, but buried under the signal itself | no |
| a sine at any level | no | no |

The second row is not hypothetical: it reproduces on the obstacle-course file
(square → sine → square) used in outside testing. So the accurate name for what
this stage does is **"minimum phase on attacks"**, not "minimum phase wherever
the filter would ring". A criterion based on measured pre-ringing is possible,
and offline it is even cheap — both branches are rendered in full before the
blend, so the ringing does not need predicting, it can be measured as their
difference — but it is not implemented in this version.

**Known limitation — the first ~24 ms of a file.** The gate is adaptive:
`1.5 × causal RMS of recent flux`, with divisor `min(i+1, context)`. For a lone
spike at frame *i* preceded by silence, the ratio of gate to flux is
`1.5/√(i+1)`, which is above one at frames 0 and 1 — the attack raises the bar
above its own value. An attack at the very start of a file does not fire, even
when it is the largest event in that file.
...
Switching: stereo-linked zero-crossing hard switch with a 1 ms fade
```
...
a ~86 Hz onset envelope using **onset flux** (positive derivative of percussive
...
Source Audio (decoded at source sample rate)
    ...
    │
    ├─→ Adaptive gate:
    │       threshold = 1.5 × causal RMS of recent flux + 0.003
    │       ← relative to the last 3 s, so quiet passages keep their own scale
    │
    ├─→ Envelope Follower:
    │       Attack:  instant
    │       Hold:    25 ms → 2 frames → 23.2 ms at 44.1 kHz
    │       Release: one analysis frame — 11.6 ms at 44.1 kHz
    │       ← the envelope lives on the analysis grid, so neither can be
    │         finer than one frame; the 8 ms the constant names is below
    │         the frame period at every source rate up to 192 kHz
    │
    ├─→ Backward Lookahead:
    │       1–2 frames of pre-roll before each onset (×0.6 and ×0.2)
    │       ≈ 11.6 and 23.2 ms at 44.1 kHz
    │       → minimum phase is already active where pre-ringing would sit
    │
    └─→ Output: <stem>.onset_envelope.json (~86 Hz at 44.1 kHz, compact)
```
...
Switch fade:       1 ms, running integral of a Hann window  (sr × 0.001)
                   e.g. 384 samples @ 384 kHz
```

The switch is a **binary hard switch with a 1 ms fade**, not a continuous crossfade blend.
Mixing the two outputs proportionally at intermediate envelope values would produce
comb filtering. The fade is applied only at the switching boundary itself.
...
**Caching:** `<stem>.onset_envelope.json` is reused only when it was written by
the current detector version **and** its `"source_fingerprint"` matches the
audio being analysed. A matching file name is not enough on its own: a source
can be edited in place, and a reused envelope would then apply the previous
file's switch points to the new audio. The fingerprint is an FNV-1a hash of the
samples rather than the file's size and timestamp, so it survives a copy or a
restore from backup and still changes when an earlier stage (headroom,
apodizer) alters what the detector sees. A mismatch is logged as
`Stale envelope cache (…) — regenerating`.
...
- Whether the ear resolves temporal artefacts at −40 … −60 dBFS in the 2–8 kHz band is argued both ways; §1 sets out why this project does not claim to have settled it
- It is why HQPlayer, Chord Mojo 2, and professional-grade converters offer
  minimum-phase and apodizing modes — as a choice, not as a verdict
- What this document establishes is the mechanism: where the switch happens the
  precursor is gone, and everywhere else the output is bit-identical to the
  linear-phase render
...
| `<source>.onset_envelope.json` | Cached HPSS envelope (~86 Hz at 44.1 kHz, fingerprinted) |
...
via median filtering, and builds the ~86 Hz onset envelope — fully eliminating the
...
3. **Tolerance for Sustain-Zone Bleed (Fade Tail)**
   - **Problem:** The engine uses a short fade (1 ms since 1.3.2) at each
   ...
   - **Fix:** The `analyze_transient` block was updated to accept `has_differences`
     ...
     sound falling inside the fade release — this is physically correct engine
     behaviour.
