...
**Update (2026-08):** the version tag alone was not enough. The sidecar is
addressed by file name, so editing a source in place and re-converting reused
the previous file's envelope and applied its switch points to different audio,
silently. The sidecar now also carries `"source_fingerprint"` — an FNV-1a hash
of the samples handed to the detector — and the cache is honoured only when
both the version and the fingerprint match. Hashing samples rather than file
size and mtime means the fingerprint survives a copy or a restore from backup,
and changes when an earlier stage (headroom, apodizer) alters what the detector
sees. Cost is bounded by construction: both ends of each channel in full plus a
fixed number of strided probes, so a 38-minute 192 kHz source costs the same
microseconds as a 3-minute one.
...
- **GPU no longer panics** on a device without `SPIRV_SHADER_PASSTHROUGH`
  (and, since 1.2.5, not when the device runs out of memory either — the
  refusal returns as an error and that filter is convolved on the CPU)
