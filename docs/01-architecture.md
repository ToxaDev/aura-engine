...
```
Decode (symphonia → f64, no clamp, gapless-trimmed)
   │
DC block (per-channel mean, or 2 Hz IIR HPF)
   │
Subsonic filter (optional, off by default): linear-phase FIR high-pass at the
...
Hybrid-Phase (optional): second convolution with the minimum-phase filter,
   HPSS onset envelope (triggers on the start of a sound, not on measured
   pre-ringing), stereo-linked zero-crossing switch between branches
   │
True-peak normalize (4× Lanczos-4 intersample; ceiling −0.5 dBTP, or
                     whatever Headroom names)
   │
...
| Decode | `symphonia` (FLAC/WAV/MP3/OGG/AAC), decoded straight to f64 — integer PCM exactly, float samples past full scale untouched |
