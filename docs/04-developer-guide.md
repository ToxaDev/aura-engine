...
The Node suite covers the one piece of frontend logic with a wrong answer that
costs something: which tap count and FS multiplier the sliders open on, given
the blobs actually installed. See
[`docs/12-precomputed-fir-matrix.md` section 8](12-precomputed-fir-matrix.md#inventory).
Everything else in `src/js` is DOM wiring, exercised by running the app.
...
manager.rs  spawns a prep thread (decode+prepare) feeding a bounded channel to
        ...
        │
pipeline/prepare.rs   decode → DC block → apodize → PreparedAudio
                      (+ the true-peak ceiling Headroom asked for)
        │
